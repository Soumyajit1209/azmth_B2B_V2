          ...
          complete: (results: Papa.ParseResult<any>) => {
            if (results.data && results.data.length > 0) {
              ...
              const headers = (results.meta.fields || [])
                ...
                .filter((header: string) => header && header !== "");
          ...
          <form onSubmit={handleSubmit}>
            <CardContent className="px-8 py-6 space-y-6">
              ...
              <div className="space-y-5">
                {contacts.map((contact, index) => (
                  <div
                    key={contact.id}
                    className="p-6 border border-gray-200 rounded-xl relative transition-all hover:shadow-sm group"
                  >
            ...
            <CardFooter className="px-8 py-5 border-t border-gray-200 flex flex-col sm:flex-row justify-end gap-3">
              ...
              <Button
                ...
                className="w-full sm:w-auto h-11 bg-primary  shadow-md"
              >
                {isSubmitting ? (
                  <>
                    <svg
